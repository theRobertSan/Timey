	...
	const sortBy = (mode, projectsToSort) => {
		...
		switch (mode) {
			case 0:
				sortedProjects.sort(
					(projectA, projectB) =>
						Number(new Date(projectA.dueDate)) -
						Number(new Date(projectB.dueDate))
				);
				break;
			case 1:
				sortedProjects.sort(
			...
			case 2:
				sortedProjects.sort(
					(projectA, projectB) => projectB.difficulty - projectA.difficulty
				);
				break;
			default:
				sortedProjects.sort(
					(projectA, projectB) => projectA.difficulty - projectB.difficulty
				);
		}
